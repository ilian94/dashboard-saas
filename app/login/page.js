    ...
    return (
      <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: 'system-ui, sans-serif' }}>
        <div style={{ width: '100%', maxWidth: '420px', background: C.card, border: `1px solid ${C.border}`, borderRadius: '20px', padding: '40px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div>
            <button onClick={() => { setShowForgot(false); setForgotSent(false); setForgotError(""); }} style={{ background: 'none', border: 'none', color: C.text, cursor: 'pointer', fontSize: '0.85rem', padding: 0, marginBottom: '20px' }}>← Back</button>
            <h1 style={{ fontSize: '1.6rem', fontWeight: 800, color: 'white', letterSpacing: '-0.03em', marginBottom: '6px' }}>Reset password</h1>
          ...
          {forgotSent ? (
            <div style={{ background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)', borderRadius: '10px', padding: '16px' }}>
              <p style={{ color: '#4ade80', fontSize: '0.9rem', fontWeight: 600, marginBottom: '4px' }}>Email sent ✓</p>
              <p style={{ color: C.text, fontSize: '0.85rem' }}>Check your inbox and follow the link to reset your password.</p>
            </div>
          ) : (
            <form onSubmit={handleForgot} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {forgotError && <p style={{ fontSize: '0.85rem', color: '#f87171', background: 'rgba(248,113,113,0.08)', padding: '10px 14px', borderRadius: '8px', border: '1px solid rgba(248,113,113,0.2)' }}>{forgotError}</p>}
              <button type="submit" disabled={forgotLoading} style={{ padding: '13px', background: '#4f46e5', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer' }}>
