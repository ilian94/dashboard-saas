        ...
        {!isMobile && activePage === 'dashboard' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', position: 'sticky', top: '40px' }}>
            ...
            <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: '16px', padding: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {!googleConnected && <a href="/api/google/auth?userId=${user?.id}?userId=${user?.id}?userId=${user?.id}" style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '10px', textDecoration: 'none', color: '#2563eb', fontSize: '0.85rem', fontWeight: 600 }}><IconCalendar /> Connect Calendar</a>}
