...
function OnboardingBanner({ plan, googleConnected, twilioNumber, onDismiss }) {
  const steps = [
    ...
    { label: 'Connect Google Calendar', done: googleConnected, action: !googleConnected ? <a href="#" onClick={(e) => { e.preventDefault(); handleConnectGoogle(); }} style={{ fontSize: '0.78rem', fontWeight: 600, color: '#1a73e8', textDecoration: 'none', whiteSpace: 'nowrap' }}>Connect</a> : null },
...
const handleDisconnectCalendly = async () => {
  ...
  const setupSteps = [
    ...
    { number: '02', title: 'Connect Google Calendar', done: googleConnected, desc: 'Link your Google Calendar so your VoiceBot can automatically book appointments in real time.', action: !googleConnected ? <a href="#" onClick={(e) => { e.preventDefault(); handleConnectGoogle(); }} style={{ display: 'inline-block', marginTop: '12px', padding: '8px 18px', background: '#1a73e8', color: 'white', textDecoration: 'none', borderRadius: '8px', fontSize: '0.85rem', fontWeight: 600 }}>Connect Google</a> : null },
          ...
          {activePage === 'dashboard' && (
            <>
              ...
              <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: '16px', padding: '20px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', gap: '12px' }}>
                ...
                {!googleConnected
                  ? <a href="#" onClick={(e) => { e.preventDefault(); handleConnectGoogle(); }} style={{ padding: '9px 20px', background: '#1a73e8', color: 'white', textDecoration: 'none', borderRadius: '9px', fontSize: '0.875rem', fontWeight: 600, whiteSpace: 'nowrap' }}>Connect Google</a>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: '#f9fafb', border: `1px solid ${activeCalendar === 'google' ? '#bbf7d0' : C.border}`, borderRadius: '12px', opacity: activeCalendar && activeCalendar !== 'google' ? 0.4 : 1, pointerEvents: activeCalendar && activeCalendar !== 'google' ? 'none' : 'auto', transition: 'opacity 0.2s' }}>
                        ...
                        {activeCalendar === 'google' ? (
                          ...
                          <a href="#" onClick={(e) => { e.preventDefault(); handleConnectGoogle(); }} style={{ padding: '8px 16px', background: '#1a73e8', color: 'white', textDecoration: 'none', borderRadius: '8px', fontSize: '0.82rem', fontWeight: 600, whiteSpace: 'nowrap' }}>Connect</a>
                        )}
                      </div>
        ...
        {!isMobile && activePage === 'dashboard' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', position: 'sticky', top: '40px' }}>
            ...
            <div style={{ background: '#fff', border: `1px solid ${C.border}`, borderRadius: '16px', padding: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {!googleConnected && <a href={`/api/google/auth?userId=${user?.id}`} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '10px', textDecoration: 'none', color: '#2563eb', fontSize: '0.85rem', fontWeight: 600 }}><IconCalendar /> Connect Calendar</a>}
...
const handleConnectGoogle = () => {
  const clientId = '501812606918-1eu5dk6qf2to36hb3b6td77rin9cifth.apps.googleusercontent.com';
  const redirectUri = 'https://www.voicebotai.us/api/google/callback';
