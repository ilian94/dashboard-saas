...
const plans = [
  ...
  },
  {
    ...
    popular: true,
    features: ['2,000 call minutes/month', '1 phone number', 'Google Calendar sync', 'AI call summaries', '24/7 availability', 'Instant response (<1s)', 'Custom business name', 'Call recording', 'Appointment modification & cancellation', 'Calendly integration', 'Analytics dashboard', 'Extra minutes from $25'],
  },
  {
    name: 'Business',
    key: 'business',
    ...
    features: ['6,000 call minutes/month', '1 phone number included', 'Google Calendar sync', 'AI call summaries', '24/7 availability + priority support', 'Instant response (<1s)', 'Custom business name', 'Full script customization', 'SMS confirmation after booking', 'Call recording', 'Advanced analytics dashboard', 'Appointment modification & cancellation', 'Calendly integration', 'Unlimited additional numbers ($15/mo each)', 'Extra minutes from $20'],
  },
];
    ...
    <div style={{ maxWidth: '680px', margin: '0 auto 64px', padding: '0 20px' }}>
      <div style={{ background: '#f9f8ff', border: '1px solid #e0e7ff', borderRadius: '20px', padding: '40px' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e0e7ff', borderRadius: '14px', padding: '24px', marginBottom: '20px' }}>
          <div className="roi-calc-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
            ...
            <div style={{ textAlign: 'center', padding: '16px', background: '#fff5f5', borderRadius: '10px' }}>
              <div className="roi-calc-value" style={{ fontSize: '1.8rem', fontWeight: 700, color: '#ef4444' }}>${lostRevenue.toLocaleString()}</div>
          ...
          <div style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', borderRadius: '10px', padding: '16px', textAlign: 'center' }}>
            ...
            <p className="roi-calc-recommended" style={{ color: '#fff', fontSize: '1.3rem', fontWeight: 700, margin: '0 0 2px' }}>{recommended} — {recommendedPrice}/mo</p>
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .roi-calc-value { font-size: 1.4rem !important; }
          .roi-calc-recommended { font-size: 1.1rem !important; }
        }
      `}</style>
    </div>
  );
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 48px', borderBottom: `1px solid ${C.border}`, position: 'sticky', top: 0, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(12px)', zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {user ? (
            <>
              <span style={{ color: C.textMuted, fontSize: '0.85rem' }} className="mobile-hide-pricing">{clientData?.business_name || user.email}</span>
      ...
      <section style={{ textAlign: 'center', padding: '72px 20px 48px' }}>
        ...
        <h1 style={{ fontSize: 'clamp(1.8rem, 4.5vw, 3rem)', fontWeight: 700, letterSpacing: '-0.04em', marginBottom: '14px', color: C.text }}>
      ...
      {/* PLANS */}
      <div className="plans-grid" style={{ display: 'flex', gap: '20px', justifyContent: 'center', flexWrap: 'wrap', maxWidth: '1060px', margin: '0 auto', padding: '0 20px 64px' }}>
        {plans.map((plan) => {
          ...
          return (
            <div key={plan.name} className="plan-card" style={{ background: C.bgCard, border: isCurrent ? '2px solid #22c55e' : isPopular ? `2px solid ${C.accent}` : `1px solid ${C.border}`, borderRadius: '16px', padding: '32px 28px', width: '300px', position: 'relative', display: 'flex', flexDirection: 'column', opacity: btn.style === 'downgrade' ? 0.55 : 1 }}>
        ...
        <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: '16px', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', padding: '14px 24px', borderBottom: `1px solid ${C.border}`, background: C.bgSecondary }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
              ...
              <span style={{ fontSize: '0.65rem', background: '#fef3c7', color: '#d97706', border: '1px solid #fde68a', padding: '2px 7px', borderRadius: '100px', fontWeight: 700 }} className="mobile-hide-pricing">BEST PRICE</span>
            </div>
          </div>
          {[
            ...
            <div key={pack.label} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', padding: '16px 24px', borderBottom: i < 2 ? `1px solid ${C.border}` : 'none', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                ...
                <span style={{ fontSize: '0.7rem', color: '#16a34a', background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '2px 7px', borderRadius: '100px', fontWeight: 600 }} className="mobile-hide-pricing">{pack.saving}</span>
              </div>
            </div>
          ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {faqs.map((faq, i) => (
            <div key={i} style={{ borderBottom: `1px solid ${C.border}` }}>
              <button onClick={() => setOpenFaq(openFaq === i ? null : i)} style={{ width: '100%', background: 'none', border: 'none', padding: '20px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', textAlign: 'left', gap: '16px', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
              ...
              {openFaq === i && <p style={{ fontSize: '0.875rem', color: C.textSecondary, lineHeight: 1.7, paddingBottom: '20px', margin: 0 }}>{faq.a}</p>}
            </div>
          ))}
        </div>
      </div>
      ...
      {/* CTA FINAL */}
      <div style={{ margin: '0 20px 80px', background: C.text, borderRadius: '20px', textAlign: 'center', padding: '64px 40px', maxWidth: '1060px' }} className="final-cta-pricing">
      ...
      <footer style={{ borderTop: `1px solid ${C.border}`, padding: '32px 48px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          {[{ label: 'Home', href: '/' }, { label: 'Sign in', href: '/login' }, { label: 'Sign up', href: '/register' }, { label: 'Privacy', href: '/privacy' }, { label: 'Terms', href: '/terms' }].map(l => (
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .mobile-hide-pricing { display: none !important; }
          .plans-grid { flex-direction: column !important; align-items: center !important; padding: 0 16px 48px !important; }
          .plan-card { width: 100% !important; max-width: 100% !important; }
          .roi-calc-grid { grid-template-columns: 1fr !important; }
          .roi-calc-value { font-size: 1.4rem !important; }
          .roi-calc-recommended { font-size: 1.1rem !important; }
          .final-cta-pricing { margin: 0 16px 60px !important; padding: 40px 20px !important; border-radius: 16px !important; }
          footer { padding: 24px 16px !important; flex-direction: column !important; text-align: center !important; }
        }
      `}</style>
    </div>
  );
