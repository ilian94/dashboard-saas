...
const plans = [
  ...
  { name: 'Scale', price: '$459', desc: 'For growing teams.', priceId: 'price_1TZI51Fbv1QHIqBxfvYIkKRh', features: ['2,000 call minutes/month', '1 phone number', 'Google Calendar sync', 'AI call summaries', 'Custom business name', 'Priority support'], popular: true },
  { name: 'Business', price: '$879', desc: 'For high-volume operations.', priceId: 'price_1TZI51Fbv1QHIqBxmfldnApH', features: ['6,000 call minutes/month', '1 phone number', 'Google Calendar sync', 'AI call summaries', 'Full script customization', 'Dedicated account manager', 'Advanced analytics'] },
];
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: 'white', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: '24px', justifyContent: 'center', flexWrap: 'wrap', maxWidth: '1100px', margin: '0 auto', padding: '0 20px 100px' }}>
        {plans.map((plan) => (
          <div key={plan.name} style={{ background: plan.popular ? C.cardPopular : C.card, border: `1px solid ${plan.popular ? C.borderPopular : C.border}`, borderRadius: '20px', padding: '40px', width: '300px', position: 'relative', display: 'flex', flexDirection: 'column' }}>
            {plan.popular && <div style={{ position: 'absolute', top: '-13px', left: '50%', transform: 'translateX(-50%)', background: '#4f46e5', color: 'white', fontSize: '0.75rem', fontWeight: 700, padding: '4px 14px', borderRadius: '100px', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>MOST POPULAR</div>}
            ...
            <button onClick={() => handleSubscribe(plan.priceId, plan.name)} disabled={loading === plan.name}
              style={{ width: '100%', padding: '13px', background: plan.popular ? '#4f46e5' : C.bg, color: 'white', border: plan.popular ? 'none' : `1px solid ${C.border}`, borderRadius: '10px', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer' }}>
